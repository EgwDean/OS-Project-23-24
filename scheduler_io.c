...
//signal handler function that enqueues process that requests i/o
void start_io_handler() {

			struct Process start_io_process; //info about the requesting i/o child to be stored here

      if (close(pipe_fdd[1]) == -1) {
      		perror("close pipe2 write end");
      		exit(EXIT_FAILURE);
      }

			if (read(pipe_fdd[0], &start_io_process, sizeof(struct Process)) == -1) { //read from pipe2
											perror("read from pipe");
											exit(EXIT_FAILURE);
			}

			struct Process* ptr;
			ptr = &start_io_process;
			enqueue(ptr, &head);
}

//signal handler function that moves process that has completed i/o to the head of the queue
void end_io_handler(){
                switch_head_tail(&head);
}
...
//------------------------------------------------------------------------------
	...
	//sact2.sa_handler = start_io_handler;
	//sact2.sa_flags = 0;
    ...
    //sact3.sa_handler = end_io_handler;
    //sact3.sa_flags = 0;
...
while (head != NULL) { //the queue still has nodes in it
      ...
      //creation of a pipe2 for communication between the child process and the signal handler
      if (pipe(pipe_fdd) == -1) {
          perror("pipe");
          exit(EXIT_FAILURE);
      }

      if (close(pipe_fdd[0]) == -1) {
      		perror("close pipe2 read end");
      		exit(EXIT_FAILURE);
      }

      //write the process information to the pipe2
      if (write(pipe_fdd[1], process, sizeof(struct Process)) == -1) {
